...
With these changes, what is the new lowest house number of the house to get at least
...
// If wanting to do more math
//  https://math.stackexchange.com/questions/22721/is-there-a-formula-to-calculate-the-sum-of-all-proper-divisors-of-a-number
func lowestHouseNumWithPresentsA(debug bool, presents int) int {
	startingHouse := presents / 50 //Speeding up.
	if startingHouse < 1 {
		startingHouse = 1
	}

	for houseNumber := startingHouse; ; houseNumber++ {
		housePresents := housePresentsA(debug, houseNumber)
		if debug {
			fmt.Printf("house[%8v] => %v \n", houseNumber, housePresents)
		}
...
func housePresentsA(debug bool, houseNumber int) int {
	presents := 0
	elfMultipler := 10
	...
	return presents * elfMultipler
}

func lowestHouseNumWithPresentsB(debug bool, presents int) int {
	startingHouse := presents / 50
	if startingHouse < 1 {
		startingHouse = 1
	}

	for houseNumber := startingHouse; ; houseNumber++ {
		housePresents := housePresentsB(debug, houseNumber, 50)
		if debug {
			fmt.Printf("house[%8v] => %v \n", houseNumber, housePresents)
		}
		if housePresents >= presents {
...
func housePresentsB(debug bool, houseNumber int, elfPresentLimit int) int {
	presents := 0
	elfMultipler := 11

	houseLimit := int(math.Ceil(float64(houseNumber) / float64(elfPresentLimit)))
	sqrt := int(math.Sqrt(float64(houseNumber)))
	if debug {
		fmt.Printf(" SQRT == %v \n", sqrt)
		fmt.Printf("House[%2v]  PresentLimit[%v] HouseLimit == %v\n", houseNumber, elfPresentLimit, houseLimit)
		for i := 1; i <= sqrt; i++ {
			if houseNumber%i == 0 {
				rem := houseNumber / i
				if i >= houseLimit {
	...
	//for i := math.Floor(sqrt); i > 0; i-- {
	for i := 1; i <= sqrt; i++ {
		if houseNumber%i == 0 {
			rem := houseNumber / i
			//if i*elfPresentLimit >= houseNumber {
			if i >= houseLimit {
				presents += i
			}
			//if rem*elfPresentLimit >= houseNumber && rem != i {
			if rem >= houseLimit && rem != i {
				presents += rem
			}
		}
