...
func deliverPresent(houses *map[int]map[int]int, x *int, y *int, directions rune) {
	if (*houses)[*x] == nil {
...
func countHousesDeliveredTo(houses *map[int]map[int]int) int {
	var houseCount int
	for _, xMap := range *houses {
		houseCount += len(xMap)
	}
	return houseCount
}

func caclHouseDeliveryAtLeastOnePresent(instructions string) int {
	var x, y int
	var houses = map[int]map[int]int{}
	houses[x] = make(map[int]int)
	houses[x][y]++
	for _, c := range instructions {
		deliverPresent(&houses, &x, &y, c)
	}
	return countHousesDeliveredTo(&houses)
}

func caclHouseDeliveryAtLeastOnePresentWithRoboSanta(instructions string) int {
	var x, y, roboX, roboY int
	var houses = map[int]map[int]int{}
	houses[0] = make(map[int]int)
	houses[x][y]++
	houses[roboX][roboY]++
	booleanRoboTurn := false
	for _, c := range instructions {
		if booleanRoboTurn {
			deliverPresent(&houses, &roboX, &roboY, c)
		} else {
			deliverPresent(&houses, &x, &y, c)
		}
		booleanRoboTurn = !booleanRoboTurn
	}
	return countHousesDeliveredTo(&houses)
}
