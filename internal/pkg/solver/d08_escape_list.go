...
"abc"
	is 5 characters of code,
	but 3 characters in the string data.
"aaa\"aaa"
	is 10 characters of code,
	but the string itself contains six "a" characters
	and a single, escaped quote character,
	for a total of 7 characters in the string data.
"\x27"
	is 6 characters of code,
	but the string itself contains just one - an apostrophe (')
...
func memoryCharsTransform(input string, debug bool) (string, error) {
	output := ""
	// Don't try to do series of replaceall (because they can effect one another incorrectly)
	// should use cursor or eval.
	//Range skips quotes
	//Having end quote is nice for if hex char is at end (for substring syntax)
	for i := 1; i < len(input)-1; i++ {

		c := input[i]
		if debug {
			fmt.Printf("\t looking at %c \n", c)
		}
		if c == '\\' {
			nextChar := input[i+1]
			switch nextChar {
			case 'x':
				hstr := input[i+2 : i+4]
				hb, err := hex.DecodeString(hstr) //You must decode!
				if err != nil {
					return "", err
				}
				hval := string(hb)
				if debug {
					fmt.Printf("hexi[%v] hstr[%v] hval[%v]\n",
						i, hstr, hval)
				}
				output += hval
				i = i + 3
			default:
				output += string(nextChar) //   \ or "
				i++
			}
		} else {
			output += string(c)
...
func calcMemoryChars(input string, debug bool) (int, error) {
	if debug {
		fmt.Printf("\ninput[%v]\n", input)
	}
	codeChars := len(input)
	output, err := memoryCharsTransform(input, debug)
	if err != nil {
		return -1, err
	}
	memChars := len(output)
	if debug {
		fmt.Printf("\ninput %-20v output %-15v codeChars %-3v memChars %-3v \n",
			"["+input+"]", "["+output+"]",
			codeChars, memChars)
	}
	return codeChars - memChars, nil
}

func calcMemoryCharsReencoded(input string, debug bool) (int, error) {
	codeChars := len(input)
	...
	encodeChars := len(output)
	if debug {
		fmt.Printf("input %-20v output %-30v codeChars %-3v encodeChars %-3v \n",
			"["+input+"]", "["+output+"]",
			codeChars, encodeChars)
	}
	return encodeChars - codeChars, nil
}
